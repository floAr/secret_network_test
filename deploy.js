...
const main = async () => {
    ...
    const wasm = fs.readFileSync('signpost/contract.wasm');
    ...
    const initMsg = { max_size: 128 };
    const contract = await client.instantiate(codeId, initMsg, `My Signpost${Math.ceil(Math.random() * 10000)}`)
